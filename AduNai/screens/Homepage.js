...
const Homepage = (navigation) => {
  ...
  const Transactions = ({item}) => {
    return (
      <View style={{width:w(330) , backgroundColor:'#fffcf3', height:h(40) ,marginBottom:h(20), display:'flex', flexDirection:'row', justifyContent:'space-between'}} >

         <View style={{padding:3, backgroundColor:'white', borderWidth:0.17, borderRadius:5}} >
         <Image source={ item.icon } />
         </View>
         
        <Text style={{color:'blue', fontSize:mh(13)}} > {item.title} </Text>
       
        <Text> {item.time} </Text>
        <Text style={{textAlign:'right', display:'flex', color:'red' }} > {item.price} </Text> 
      </View>
    )
  };
      ...
      <View style={styles.fbox}  >
        <View >
          ...
          <Image style={{margin:mh(10) }} source={require('../profile.png')}  />
                  ...
                  {/* Green Active Button */}
          <View style={{height:h(16), width:w(16), borderRadius:50 , backgroundColor:'yellow', zIndex:99, top:50, left:50,
          position:'absolute'}} />
